...
namespace Encoder {
    ...
    bool initialize() {
        ...
        for (int i = 0; i < DATASET_SIZE; i++) {
            ...
            /**
             * @details Can't use rpmSamples[i] == NAN, as caught by PVS Studio
             * (Rule V550, CWE-682)
             * @see
             * https://stackoverflow.com/questions/570669/checking-if-a-double-or-float-is-nan-in-c
             */
    ...
    etl::string<Encoder::LOG_STRING_SIZE> getLogString() {
        ...
        etl::to_string(as5600.getAddress(), logString, hexFormatA,
                       true);            // 2 chars
        (void)logString.append(", D: "); // 5 chars

        etl::format_spec boolFormatA;
        etl::to_string(as5600.getDirection(), logString, boolFormatA,
                       true); // 1 char
        ...
        etl::to_string(as5600.magnetTooStrong(), logString, decFormatB,
                       true);              // 1 chars
        (void)logString.append(", MTW: "); // 6 chars
        etl::to_string(as5600.magnetTooWeak(), logString, decFormatB,
                       true);             // 1 chars
        (void)logString.append(", lE: "); // 6 chars

        etl::format_spec decFormatC;
        (void)decFormatC.width(4).fill('0'); // [4 chars]
        etl::to_string(as5600.lastError(), logString, decFormatC,
                       true); // 4 chars
