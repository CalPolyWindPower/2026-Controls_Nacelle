...
/**
 * @brief Constructor. Initializes member variables and sets the singleton
 * instance pointer.
 */
...
esp_err_t NacelleComms::setupPeer_() {
    ...
    if (result != ESP_OK) {
        ESP_LOGE(TAG, "Failed to add peer: %d at %02x:%02x:%02x:%02x:%02x:%02x",
                 result, LOADBOX_MAC[0], LOADBOX_MAC[1], LOADBOX_MAC[2],
                 LOADBOX_MAC[3], LOADBOX_MAC[4], LOADBOX_MAC[5]);
    } else {
        ESP_LOGI(TAG, "Peer added: %02X:%02X:%02X:%02X:%02X:%02X",
                 LOADBOX_MAC[0], LOADBOX_MAC[1], LOADBOX_MAC[2], LOADBOX_MAC[3],
                 LOADBOX_MAC[4], LOADBOX_MAC[5]);
    }
 ...
 */
void NacelleComms::onDataSent_(const wifi_tx_info_t *tx_info,
 ...
 */
void NacelleComms::onDataRecv_(const esp_now_recv_info_t *recv_info,
    ...
    if (len == sizeof(LoadboxPacket)) {
        ...
        // instance_->remoteActuatorPos_ =
        // instance_->incomingPacket_.actuatorPos; ESP_LOGI(TAG, "Rx success");
 ...
 */
bool NacelleComms::isLinkAlive() const { return linkAlive_; }
